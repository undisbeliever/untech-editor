#ifndef _UNTECH_GUI_WIDGETS_METASPRITE_PALETTECOLORDIALOG_HPP_
#define _UNTECH_GUI_WIDGETS_METASPRITE_PALETTECOLORDIALOG_HPP_
...
public:
    ColorArea(Snes::SnesColor& color)
        : Gtk::DrawingArea()
        , _color(color)
    {
    }
...
protected:
    ...
    {
        auto& rgba = _color.rgb();

        cr->set_source_rgba(rgba.red / 255.0,
                            rgba.green / 255.0,
                            rgba.blue / 255.0,
                            1.0);
...
class PaletteColorDialog : public Gtk::Dialog {
    static const unsigned MAX_COLOR = 31;

public:
    PaletteColorDialog(MS::Palette& palette, unsigned colorId, Gtk::Widget& parent)
        : Gtk::Dialog(_("SNES Color"), false)
        , _grid()
        , _blueScale(Gtk::Adjustment::create(0.0, 0.0, MAX_COLOR, 1.0, 4.0, 0.0),
                     Gtk::ORIENTATION_HORIZONTAL)
        , _greenScale(Gtk::Adjustment::create(0.0, 0.0, MAX_COLOR, 1.0, 4.0, 0.0),
                      Gtk::ORIENTATION_HORIZONTAL)
        , _redScale(Gtk::Adjustment::create(0.0, 0.0, MAX_COLOR, 1.0, 4.0, 0.0),
                    Gtk::ORIENTATION_HORIZONTAL)
        , _colorArea(palette.color(colorId))
        ...
        , _updatingValues(false)
    {
        ...
        signal_response().connect([this](int response) {
            if (response == Gtk::RESPONSE_ACCEPT) {
                Private::palette_setColor(_palette, _colorId,
                                          _oldColor,
