...
public:
        ...
        , _grid(std::move(data))
    {
        if (_grid.size() != _width * _height) {
            throw invalid_argument(u8"grid data size must equal width * height");
        }
    }
    ...
    {
        if (y >= _height) {
            throw out_of_range(u8"grid::scanline out of range");
        }
...
private:
    ...
    {
        if (x >= _width) {
            throw out_of_range(u8"grid: x (", x, u8") >= width (", _width, u8")");
        }
        if (y >= _height) {
            throw out_of_range(u8"grid: y (", y, u8") >= height (", _height, u8")");
        }
    }
    void _rangeCheck(unsigned x, unsigned y, unsigned width, unsigned height) const
    {
        _rangeCheck(x, y);
        if (x + width > _width) {
            throw out_of_range(u8"grid: x + width (", (x + width), u8") > width (", _width, u8")");
        }
        if (y + height > _height) {
            throw out_of_range(u8"grid: y + height(", (y + height), u8") > height (", _height, u8")");
        }
    }
};
}
